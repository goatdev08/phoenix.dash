...
Versión 0.2.0 | Entorno Ubuntu 24.04 LTS · Deploy en Streamlit Cloud (GitHub CI)
-------------------------------------------------------------------------------
Este archivo es el punto único de verdad de la aplicación. Cada incremento
mayor/menor/de parche deberá reflejarse leyendo el archivo VERSION o, si éste
no existe, usando el valor en APP_VERSION. Al etiquetar un commit en GitHub,
actualiza VERSION y la constante APP_VERSION se ajustará automáticamente.
"""
...
with st.sidebar:
    ...
    st.session_state["nadadores"] = st.multiselect(
        "Selecciona hasta 4 nadadores:", df.Nadador.unique(),
        default=st.session_state["nadadores"], max_selections=4,
    )
    ...
    st.session_state["parametros"] = (
        ...
        else st.multiselect("Parámetro(s):", df.Parametro.unique(), default=st.session_state["parametros"])
    )
...
st.sidebar.download_button(
    "📥 Descargar datos filtrados", csv_data, file_name=f"Phoenix_filtered_{datetime.now():%Y%m%d}.csv", mime="text/csv"
)
    ...
    for category, param_list in param_categories.items():
        ...
        for parametro in selected_in_category:
            ...
            for idx, estilo in enumerate(estilos_unicos):
                ...
                show_legend = not is_mobile or st.checkbox("Mostrar leyenda", key=f"legend_{parametro}_{estilo}")
                fig.update_layout(
...
with tab2:
    if st.session_state["nadadores"]:
        for nadador in st.session_state["nadadores"]:
            ...
            sub_df["Parametro"] = (
                sub_df["Parametro"].map(param_translation).fillna(sub_df["Parametro"])
            )
